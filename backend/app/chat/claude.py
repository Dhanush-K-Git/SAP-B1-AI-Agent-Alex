...
_SQL_SYSTEM = (
    ...
    "- OITM (Items): add T1.\"frozenFor\" = 'N' — 'N' means active/not discontinued. DO NOT use validFor on OITM.\n"
    "- OCRD (Business Partners — customers/vendors): add T1.\"frozenFor\" = 'N' — excludes inactive BPs.\n"
    ...
    "ALWAYS include the status as a readable column in SELECT when showing master data:\n"
    "- OITM: CASE WHEN T1.\"frozenFor\"='Y' THEN 'Inactive' ELSE 'Active' END AS \"Status\"\n"
    "- OCRD: CASE WHEN T1.\"frozenFor\"='Y' THEN 'Inactive' ELSE 'Active' END AS \"Status\"\n"
    "EXCEPTION: If the user explicitly asks for 'inactive', 'all items including inactive', "
    "'active and inactive', or 'discontinued' — REMOVE the frozenFor filter so both appear, "
    "but still include the Status column so the user can see which is which.\n\n"
...
_SUMMARY_SYSTEM = (
    ...
    "OUTPUT EXACTLY THESE 2 BLOCKS IN ORDER — NOTHING ELSE. No title. No prose summary.\n\n"

    "BLOCK 1 — DATA TABLE\n"
    "Write one clean Markdown table immediately — no heading, no intro sentence before it.\n"
    "Rules:\n"
    ...
    "- Format all currency values in Indian format: ₹ symbol, Indian comma notation "
    "(e.g. ₹2,60,22,142 not ₹26,022,142). Use Lakhs/Crores labels where helpful.\n"
    "- Do NOT bold cell values inside the table.\n"
    "- Add a TOTAL row at the bottom only if it adds clear value.\n\n"

    "BLOCK 2 — KEY INSIGHTS\n"
    "Write the words KEY INSIGHTS on its own line, then exactly 3 bullet points.\n"
    "Rules for EACH bullet:\n"
    "- One sentence only.\n"
    "- Must contain at least one specific bold figure in Indian format "
    "(₹ symbol + Indian commas, or X Lakhs / X Crores).\n"
    "- Reference Indian financial context where relevant: Indian FY runs April–March, "
    "mention GST if tax figures appear, use Lakhs/Crores for large numbers.\n"
    "- No pipe characters ( | ) anywhere in bullets.\n"
    "- No table rows inside bullets.\n"
    "- No titles or section headings inside bullets.\n\n"

    "STOP after the third bullet. Write nothing after it.\n"
    "Do NOT write a title. Do NOT write a summary paragraph. Do NOT write KEY INSIGHTS twice.\n\n"
    ...
    "═══ INDIAN FINANCIAL CONTEXT RULES ═══\n"
    "- Currency symbol: ₹ (not Rs. or INR).\n"
    "- Number format: Indian comma system — 1,00,000 (1 Lakh), 10,00,000 (10 Lakhs), "
    "1,00,00,000 (1 Crore). Never use Western millions/billions.\n"
    "- Financial year: April to March (e.g. FY 2024-25 = April 2024 – March 2025).\n"
    "- When tax amounts appear, reference GST context (CGST/SGST/IGST).\n"
    "- Budget comparisons: use terms like 'actuals vs budget', 'variance', 'utilisation %'.\n\n"

    "═══ FORMAT RULES ═══\n"
    "- Percentages: +27.6% with sign. Bold in bullets.\n"
